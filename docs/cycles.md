...
- **Tests.** `tests/test_cycle_a.py` (14): full recovery lifecycle (generate → recover → reuse fails), password change invalidating codes, wrong-code 401, profile update paths, admin role gates, last-admin guard, self-deactivation guard, deactivate→login-fails, body-match snippets, title-over-body ranking. Suite: **83 passing**.
