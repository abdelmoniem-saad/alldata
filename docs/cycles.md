...
## S, Hygiene & hardening (audit-driven)

**Intent.** With the catalog complete (R), a whole-codebase audit replaced the usual feature/content planning: one failing test, a dishonest security surface (`/api/execute` claimed rate limits it never enforced, and could run code unsandboxed on the host), two silently-broken legacy block types (`quiz` fell to a markdown dump, dropping its solution), zero crash isolation, no CI despite docs claiming it, and two lessons (`p-values`, `exploratory-data-analysis`) far below the bar everything else meets. S fixes what the audit found, and sets the post-S roadmap: T = perf/polish (bundle), U = Pyodide, V = product features.

**Shipped.**

- **S0, test repair.** `get_leads_to` returns `PrerequisiteEntry{node, why}` (G8); the test still read `.slug`. One-line fix; suite went 1F/21P → 22P.
- **S1, `/api/execute` hardened.** Auth required (`CurrentUser`); per-user sliding-window rate limiter honoring the config tiers (10/min learner, 60/min professor/editor/admin) with 429 + `Retry-After`; the unsandboxed local fallback gated behind `SANDBOX_ALLOW_LOCAL_FALLBACK` (loud startup warning while on; startup *refuses* if the dev `secret_key` survives into production posture). Frontend degrades honestly: anonymous auto-runs are skipped, manual Run shows "sign in to run code." Verified live: anonymous → 401; 11th rapid learner call → 429 `Retry-After: 40`.
- **S2, crash isolation.** One `ErrorBoundary` class (the only class component, by necessity), two altitudes: block-level around every plot/flythrough mount (BlockRenderer + ScrollReader's pinned pane, resetting per anchor) and page-level around Layout's `<Outlet/>` (resetting on navigation). Verified by making a spec throw: themed "visualization error" panel, lesson scrolls on, navbar lives.
- **S3, Zustand snapshot fix.** Three selectors returned a fresh `{}` for uninitialized topics, the source of the "getSnapshot should be cached" warning on every playground lesson. One frozen module-level `EMPTY_STATE` in `topicState.ts`, used by `useTopicState` and `PlaygroundBlock`. Verified console-clean across slider-driven re-renders.
- **S4, quiz retired; two lessons rebuilt.** `p-values` rewritten to the concept bar, the pinned `gaussian_pdf(n)` playground teaches the under-taught fact that p depends on evidence, not just effect (same 0.4 lift: p≈0.69 at n=1 → p≈0.046 at n=25), and the simulation shows p uniform under a true null (deciles all ~10%). `exploratory-data-analysis` rewritten to the practice bar; its old quiz became the mean-vs-median decision. The importer now warns (→ strict error) on any `quiz` block; zero remain in seed.
- **S5, tests 22 → 53.** `test_parser.py` (16 cases: directives, YAML bodies, layers, the TODO + quiz guards, plot-bind validation), `test_forks.py` (8: fork lifecycle incl. real-seed sourcing, suggestion snapshot semantics, review queue), `test_execute_limit.py` (7: window mechanics with injectable clock, no sleeps, no Docker).
- **S6, CI.** `.github/workflows/ci.yml`: backend (pytest + full `--strict` import on a scratch DB) + frontend (tsc + production build). Both verified locally; authoring.md's "CI runs it" is finally true.
- **S7, docs.** features.md (execution hardening, crash isolation, quiz guard, API table), this retrospective.

**Retrospective.**

- **Audit-then-fix is a real cycle shape.** Z→R always planned forward (next feature, next content). S planned *backward* from evidence: every item traced to a verified finding (a failing command, a grep, a missing file), which made scoping uncontroversial and verification self-evident, each fix's test is the finding that exposed it.
- **The docstring was the vulnerability.** `/api/execute`'s comment promised limits the code never had, worse than no claim, because readers (and config) trusted it. The honest version cost ~60 lines. Same lesson as M's content-count drift, now in security clothing: a claim that isn't enforced by code or CI will silently go false.
- **Auth changes ripple to UX.** Requiring sign-in for execution would have broken every anonymous reader's auto-run sims with bare 401s. The backend fix needed two frontend lines (skip doomed auto-runs, translate the 401) to stay humane, grep for consumers before hardening an endpoint.
- **Write tests against observed semantics, not assumed ones.** Two of three initial test failures were wrong *expectations*: the parser's default layer is `both` (not `intuition`), and playground binds *declare* keys (only plot binds are validated against declarations). The failing tests documented the real contracts better than the docstrings did.
- **The bundle number is now precise.** CI's build step put the T-cycle baseline on record: **871 kB JS / 262 kB gzip, single chunk**, Vite itself warns. T's code-splitting has a measurable target.

**Deferred (carry-forward).** The optional gear retrofit for `bayes-theorem` + `conditional-probability` (rich content, cosmetic dividers only). vitest for frontend pure-logic modules (revisit in T with the plots split). Redis-backed limiting if deployment ever goes multi-process. Then the standing roadmap: **T** (code-splitting to <300 kB initial, plots/index.tsx split, a11y quick wins), **U** (Pyodide in-browser Python), **V** (about/PWA/OG, fork chains, coverage dashboard, high-contrast/WCAG, schema expansion).

## T, Performance & polish

**Intent.** S's CI build recorded the baseline that triggered T: **871 KB JS / 262 KB gzip in a single chunk**, Vite warning. The cause was `App.tsx` statically importing all 11 pages, so d3 (force graph + 23 plot specs), katex's JS, and react-markdown all rode in the bundle a first-time visitor downloaded before seeing Home. T makes the initial load lean and ships the accessibility quick-wins, and, on inspection, *reshaped its own roadmap bullets* rather than executing them literally.

**Shipped.**

- **T0, route-level code-splitting (the win).** `React.lazy` for every route except the `Layout` shell and `Home` landing; one `<Suspense>` nested inside the S2 page `ErrorBoundary` (so a failed chunk-load degrades to the themed panel, not a hung spinner). **Initial entry chunk 871 → 214 KB raw (262 → 68 KB gzip), −75%; the >500 KB warning is gone.** The lesson surface (`ScrollReader` + plots + katex, 492 KB) loads on first `/topic`/fork visit; the force graph (76 KB) on first `/explore`; both cached. Verified: Home + `/explore` + a lesson all render, the curve still reacts to the slider, console clean.
- **T1, dead-dep removal.** codemirror + 3 related packages were in `package.json` but imported nowhere (CodeRunner uses a `<textarea>`). Dropped; lockfile refreshed. Lighter install, honest manifest.
- **T2, bundle audit.** Numbers above, recorded in `features.md`. The gated d3-submodule slimming was **not** triggered: the on-demand `ScrollReader` chunk is dominated by katex (non-optional for math), not d3, so submodule imports wouldn't move the needle.
- **T3, accessibility quick-wins, narrowed to the real gaps.** Plot SVGs now present as one labeled image (`role="img"` + a `SPEC_LABELS` map in `PlotBlock`, collapsing d3 internals out of the a11y tree); `GraphFlythrough` labeled likewise; playground sliders gained `aria-label` + live `aria-valuetext`; `prefers-contrast: more` lifts the sub-AA muted text in both themes. Verified live: the labels and value-text are present and update.
- **T4, docs.** `features.md` Performance + Accessibility entries (with the chunk numbers and the not-split rationale); this retrospective.

**Retrospective.**

- **The headline roadmap task was the wrong one, and the audit said so before any code.** "Split `plots/index.tsx` into per-family files" was the cycle's marquee item; tracing the import graph showed it's perf-**neutral** (the `PLOT_SPECS` registry eagerly references every spec, so Rollup chunks them together regardless of file count) while actively harmful to maintainability (shared `lgamma`/`normCdf`/`betaPdf` helpers; `gaussian_pdf`/`student_t_pdf`/`empirical_histogram` span families). The user had flagged exactly this instinct. Lesson: "split the big file" is a code-*smell* heuristic, not a bundle-*size* lever, verify which one you're actually solving. Documented the decision so it isn't re-proposed.
- **Three of four roadmap bullets dissolved on contact with the code.** codemirror was already unused (textarea, not an editor); katex CSS was already a CDN link; `:focus-visible` and keyboard-operable decision buttons already shipped in J6. The real work was one lever (route-splitting) plus three small a11y touches. Reading before planning turned a four-part cycle into a one-big-win cycle.
- **`React.lazy` did the chunking for free.** No `manualChunks` config, no per-spec dynamic imports, just lazy routes, and Rollup hoisted shared deps (d3) into the right async chunks on its own. The 75% initial cut came from ~12 lines in `App.tsx` + a Suspense in `Layout`. The cheapest big win of any cycle.
...
## V, Public face & accessibility  (the V backlog, evaluated)

**Intent.** "V, product features" was a 7-item carried backlog. The first job was to weigh each item against the project's values (simplicity, smoothness, no third-party reliance, low maintenance) and actual state (catalog 100%; solo-built, no contributor community; server-bound, not static; S1 already hardened execution; T3 already shipped a11y quick-wins). **Most of the list is premature or marginal**, the cycle shipped only the items that pass cleanly, plus three owner-reported UX fixes.

**Per-item verdict (recorded so the deferred items aren't silently lost):**
- **/about page**, *did* (cheap public "why" from the existing identity/vision docs).
- **PWA manifest + maskable icons**, *skipped*: the app is server-bound (auth/progress/graph), not offline-capable; a manifest offers an "install" of something that won't work offline, and a real service worker is a cache-versioning maintenance trap.
- **Per-topic dynamic OG images**, *skipped*: needs a render pipeline (headless-browser dep or 40+ build-time PNGs; P confirmed no rasterizer in-env) for marginal share value.
- **Fork chains / signal aggregation / per-fork graph edits**, *deferred*: premature collaboration infra with no contributor community; per-fork graph edits is a large model change (the graph is global/seed-defined).
- **Content-coverage dashboard**, *skipped*: catalog is 100%, so it'd render all-green; revisit during schema expansion.
- **High-contrast theme + full WCAG**, *did the bounded half* (a selectable high-contrast theme + targeted core-path fixes); the open-ended full audit stays deferred.
- **Schema expansion**, *flagged as the real high-value next track*: it's content, not a feature; reuses the Q/R machinery; needs an owner domain pick (time series? causal inference? GLMs?).
- **LTI / mentor / community / WebR**, *deferred*: presuppose institutions/a community that don't exist; WebR is Pyodide-for-R, already declined in U.

**Shipped.**
- **V0, three owner-reported fixes.** (1) The sign-in modal didn't appear until you hovered the navbar, on topic pages the navbar's `.navbar-auto-hide` `transform` made the `position:fixed` modal a descendant of a transformed containing block, hiding it; **`createPortal(…, document.body)`** fixes it. (2) The pinned viz hugged the top, sticky offset `header+32px` → `header+14vh`, resting it between top and center. (3) Running R hit "R is not installed", a new no-auth `GET /api/execute/capabilities` probes Docker/`Rscript` and `CodeRunner` hides the **R** toggle where R can't run, so the dead end never appears.
- **V1, `/about`** lazy route (3.3 KB chunk) from `docs/identity.md` + `docs/vision.md`, linked in the navbar.
- **V2, selectable high-contrast theme.** `themeStore` → `dark | light | high-contrast`, navbar control cycles all three; a dark-based `[data-theme='high-contrast']` token block (pure black, near-white text at every step, `#7a7a7a` borders, brighter accent, luminance-spread domain hues). Persists across reload.
- **V3, core-path WCAG.** CodeRunner output is an `aria-live` region; `/explore` gained a visually-hidden, tab-order/AT-reachable "All topics" `<nav>` (40 links) so the force graph isn't a keyboard dead end. The open-ended `design:accessibility-review` audit folds into the deferred full-WCAG track.
- **V4, docs.** features.md (the above) + this retrospective.

**Retrospective.**
- **"Evaluate before executing" earned its keep a third time.** After the plots-split (T) and Pyodide (U), V was the clearest case yet: 5 of 7 backlog items were premature or marginal on inspection, and naming *why* in the ledger is as valuable as the code, it stops them being silently re-attempted. A backlog is a list of hypotheses, not a queue of commitments.
- **The owner's live fixes outranked the roadmap's features.** The three things the owner actually hit (hidden modal, viz position, R dead end) were worth more than most of the planned backlog, and V0 prioritized them. Ship what's in the user's way first.
- **A CSS `transform` silently captures `position:fixed`.** The modal bug was U0's making, lifting sign-in to be page-invocable while the modal still lived inside the auto-hide navbar, whose `transform` is a containing block for fixed descendants. Portaling to `document.body` is the durable pattern for any overlay that can be summoned from inside a transformed/animated subtree.
- **"Don't offer what can't run."** The R dead end is gone not by making R work (it can't, locally) but by *probing capability and hiding the option*. A capability endpoint that the UI gates on is the honest fix for any optional, environment-dependent feature.
- **High contrast is bounded only if you draw the line.** A selectable theme + targeted fixes is shippable; "full WCAG audit" is open-ended. Splitting the genuinely-valuable bounded half from the open-ended half kept V small, the same move as T's "quick wins now, full audit later."

**Deferred (carry-forward).** Everything skipped above (PWA, dynamic OG, fork chains, coverage dashboard, full WCAG audit incl. the `design:accessibility-review` pass and non-hue domain encoding, LTI/mentor/community/WebR). The genuine next major investment is **schema expansion**, new domains on the Q/R content machinery, pending an owner decision on which domain to add. Also still open: d3-submodule slimming (gated), the optional gear retrofit, a vitest harness, Redis-backed limiting if deployment ever goes multi-process.

## W, Fork authoring studio  (the R answer; fork-discovery deferred)

**Intent.** Three candidate directions were raised, each to be weighed against the platform identity (`identity.md`: "Statistics is a graph, not a textbook"; guided simulation; **"not gamified, no streaks, points, badges, or leaderboards"**) and the actual code before any of it was built:
1. *"Why isn't R there like Python?"*
2. Turn the fork editor's raw textarea into a guided authoring surface (toolbar + a picker showing every graph type).
3. Public forks + a fork-centric graph view + a "GitHub-star" endorsement to suggest good forks.

**The three-way verdict (recorded so the deferred parts aren't re-proposed):**
- **(1) R, a small honesty fix, not a build.** R was never provisioned *anywhere*: no `Rscript` locally, no R image (the base `Dockerfile.sandbox` is Python-only), and **zero authored R content**. The `_execute_r` path and `sandbox_r_image` config were aspirational scaffolding that never ran. Making R first-class is real infra + eventual content for **no current payoff** (nothing to run). So W just made the runtime story *truthful*: a `sandbox_r_enabled` flag, a capability probe that reports R only with a real runtime, and a concrete `Dockerfile.sandbox-r` enablement path. Stops there until R content actually exists.
- **(2) Fork authoring studio, the headline; built.** The fork + merge-back system (N/O) is fully wired, but to contribute you had to already know the arcane directive vocabulary (`<!-- block: plot, spec:…, binds:… -->`, `gear`, `decision`, `playground`). That barrier is *why the contribution system sees no use.* A toolbar that inserts correct scaffolds and a picker that surfaces the 23 specs makes the platform's unique authoring vocabulary discoverable, squarely serving "the graph is editable, remixable content."
- **(3) Public forks + fork-graph + stars, deferred, and the star mechanic is off-identity.** Three reasons: **(a)** stars are a popularity/leaderboard mechanic that `identity.md` explicitly rules out, and the identity-aligned "good forks rise" path *already exists*: **merge-back** (suggest → reviewer accepts → becomes master); **(b)** it presupposes an active fork community that doesn't exist yet (the same prematurity that deferred V's fork-chains); **(c)** it's built on absent foundations, forks have no `visibility` field (all implicitly public), no per-topic fork listing, no endorsement model, so it's a large multi-part build (visibility + endpoints + a new graph layout + a star table). The *navigation* half ("view a topic's public forks") could return later, **star-less**, once forking is actually used.

**Shipped.**
- **W0, R runtime made honest.** `settings.sandbox_r_enabled` (default `False`); `runtime_capabilities()` now reports `r = bool(Rscript on PATH) or sandbox_r_enabled`, Docker presence no longer implies R, because the base image ships none. New `infra/Dockerfile.sandbox-r` (`FROM r-base`, a small `ggplot2`/`dplyr`/`tidyr` set, non-root user) is the concrete enablement path the `_execute_r` code already expected. Unit test covers the flag.
- **W1, directive toolbar.** A strip over the source textarea inserts canonical scaffolds at the cursor (H2 / bold / italic, Section, Callout, Misconception, Decision, Playground, Simulation, State). Block inserts self-frame with blank lines and leave the caret after them; inline buttons wrap the selection. `frontend/src/components/topic/ForkEditorToolbar.tsx`.
- **W2, plot picker.** The **Plot…** button opens a modal listing all 23 specs grouped by family (label + bound keys) plus a `graph_view` step; picking inserts a `state` + `plot` pair seeded with defaults + a generated anchor, live in the preview. The label/params/binds metadata was promoted out of `PlotBlock` into a shared `frontend/src/lib/plotSpecs.ts` that the picker and the renderer's a11y labels both read. Portal-rendered like the auth modal.
- **W3, editor affordance polish.** Toolbar styled into editor chrome; a hover **? reference** cheat-sheet of the directive vocabulary; an "edits render live →" hint pointing at the preview.

**Retrospective.**
- **"Evaluate before executing", a fourth time, and the sharpest split yet.** Of three directions, one was a misframed question (R was never there to break), one was the real win, and one ran against the stated identity. Naming *why* each landed where it did is the durable output, especially "stars conflict with no-gamification, and merge-back already does this job," so it isn't re-proposed as a feature.
- **The barrier to a built system was discoverability, not capability.** Forking, editing, live-preview, and merge-back all worked, but went unused because the *authoring vocabulary* was invisible. The cheapest lever on an unused feature is often surfacing what it can already do, not adding more.
- **A capability question can have "it was never provisioned" as its honest answer.** R "missing" wasn't a bug to fix; it was infra that never existed for content that was never written. The right move was to make the story truthful (probe + flag + a real Dockerfile) rather than fake first-class support for an empty track.
- **Parser-validity is the verification that matters for an insert tool.** The risk wasn't pixels, it was whether inserted scaffolds *parse*. Feeding the exact toolbar/picker output through `parse_content_md` + the `--strict` validator (9 block types, zero warnings) proved the real contract directly; `tsc` + `build` covered the wiring.
- **One catalog, two readers, the `PLOT_SPECS` discipline scales.** Promoting the spec metadata into `lib/plotSpecs.ts` (consumed by both the picker and `PlotBlock`'s labels) repeats the existing frontend-registry / importer-`_KNOWN_PLOT_SPECS` mirror, so a new spec still has one obvious place to land its label/binds.

**Deferred (carry-forward).** Public-fork **discovery** (a star-less "view this topic's forks" view), revisit only if forking sees real use, and even then without a leaderboard mechanic; it needs net-new foundations (a `visibility` field, per-topic fork listing, a fork-graph layout). **First-class R content**, a separate content decision once the runtime is provisioned (build `Dockerfile.sandbox-r`, set `SANDBOX_R_ENABLED`). A **WYSIWYG** fork editor stays rejected (the source must remain plain `content.md` for merge-back diffs). Still open from prior cycles: **schema expansion** (the genuine next major investment, new domains on the Q/R machinery, pending an owner domain pick), d3-submodule slimming (gated), the optional gear retrofit, a vitest harness, Redis-backed limiting if multi-process.

## X, Authoring studio II: the block editor  (hide the plumbing)

**Intent.** W made the directive vocabulary *insertable*, but the fork editor's left pane was still a raw `content.md`, a wall of `<!-- block: gear, n: 3, label: "…" -->` and YAML bodies. The owner's objection was twofold: it's **overwhelming**, and it **leaks the implementation** (a contributor shouldn't read the platform's plumbing to add a paragraph). The cycle's job: hide the directive machinery without breaking anything downstream.

**The model decision (recorded so the rejected paths aren't re-proposed).**
- **Obsidian-style live-preview, rejected.** It hides *markdown formatting marks* (`**`, `#`), not *custom block machinery*; our `<!-- block -->` comments would render as raw text. Faking it needs custom CodeMirror widget plugins on a third-party editor we deliberately removed in T.
- **Third-party WYSIWYG, rejected.** Heavy dependency (against the no-third-party / low-maintenance values), and its lossy markdown round-trip would **break merge-back**, which diffs the plain `content.md`.
- **Home-grown block editor (Notion block-widget model), chosen.** Prose stays editable markdown (writing, not plumbing); every directive becomes a friendly card/form; raw is shown only via an opt-in per-block "edit source" hatch.

**The safety property that made it tractable.** The editor is a *pure frontend editing layer over the same `markdown_source` string*, **no backend, parser, schema, or merge-back change.** A frontend tokenizer partitions the source into segments whose `raw` text concatenates back **byte-for-byte** (`serializeDoc(parseDoc(x)) === x`). Editing one block rewrites only that segment; untouched segments stay verbatim, so a one-block edit is a one-block diff. This invariant, verified over all 46 seed `content.md` files (600+ directive segments, zero mismatches), is the whole reason the model is safe. Round-tripping through the *backend* parser would have been lossy; round-tripping through a *partition* tokenizer is exact by construction.

**Shipped.**
- **X0, `contentDoc.ts`.** `parseDoc`/`serializeDoc` over the raw string, mirroring the backend directive regexes/type-sets (`seed/import_seed.py`). Best-effort `parseAttrs` for display/forms (never affects round-trip). Verified by an esbuild-bundled node check over every seed file.
- **X1, `BlockListEditor` + `BlockCard`.** The Visual pane: prose runs as auto-growing markdown fields; directives as cards (type tag + plain-language summary from `directiveMeta.ts`, reorder/delete/edit). Whitespace/`---`/`<!-- layer -->` separators render as compact rules/chips, never empty boxes.
- **X2, per-directive forms.** Friendly editors for gear, layer, callout, derivation, misconception, state, graph_view, code/simulation, and plot, each emitting canonical directive text via `blockForms/emit.ts`. `decision`/`playground` (and any unmapped type) use the raw escape hatch.
- **X3, `ForkEditor` toggle.** A Visual ⇄ Source segmented control (Visual default); both bind the same `source` state → the existing debounced `/forks/preview` + Save, untouched.
- **X4, polish/a11y.** Editor chrome; real `<button>` reorder controls with `aria-label`s; `aria-expanded` edit toggles; labeled form fields; compact dashed layer chips.

**Retrospective.**
- **"Hide the plumbing" had a precise meaning worth nailing before coding.** The instinct was "make it like Obsidian," but Obsidian hides markdown marks, not custom directives, it would *not* have solved the complaint. Naming the Notion *block-widget* model vs the Obsidian *live-preview* model up front pointed at the only design that actually hides the machinery.
- **A partition tokenizer beat re-serializing the parser.** The scary version of this cycle was "round-trip through the backend parser," which is lossy and would churn merge-back diffs. Modeling the document as exact source slices (`serializeDoc(parseDoc(x)) === x`) made the round-trip free and the diff minimal, and let the entire feature be frontend-only, with the 57 backend tests green by construction.
- **The escape hatch is what makes the cycle shippable.** Visual mode doesn't have to handle every directive perfectly on day one: a Source toggle and a per-block raw editor mean `decision`/`playground` (and anything new) degrade gracefully to raw text while the common 90% of blocks get friendly forms. Ship the forms that pay off; fall back for the rest.
- **The leak you don't see in code, you see in the browser.** Round-trip identity and `tsc` were both green while two prose boxes still showed `<!-- layer: formal -->`. Only opening the editor in the preview surfaced it; lifting layer markers into their own chip closed the last gap. Verify the *experience*, not just the invariants.

**Deferred (carry-forward).** Friendly forms for `decision`/`playground` (currently raw-hatch); drag-and-drop reorder (↑/↓ buttons first); inline-rendered plot thumbnails in cards (the right pane is the live render). Everything carried from prior cycles stands: **schema expansion** as the next major content track, plus the long-tail (d3 slimming, vitest harness, Redis limiting if multi-process). Fork-discovery + stars remain deferred for the W reasons (off-identity; premature).

---

## Y, Runtime hardening & repo hygiene

**Intent.** A security review of the actual production host (the Hugging Face Space) surfaced three exposures: the Space can't nest Docker, so `/api/execute` falls back to running submitted code on the *host interpreter* with only rate limits and a wall-clock timeout between untrusted code and the server's secrets; the auth endpoints had no rate limiting at all (no email path exists, so password guessing was unthrottled); and the repo carried drift (chat-transcript leftovers in the README, an empty Alembic scaffold contradicting Principle 7, a stray nested HF clone, no LICENSE). Also: `load(name)` silently failed in both execution paths because it resolved datasets relative to a cwd that a temp-dir runner or sandbox container doesn't have.

**Shipped.**
- **Y0, repo hygiene.** README rewritten (identity-accurate, no chat leftovers); MIT `LICENSE` added; `alembic/` + `alembic.ini` deleted and the `alembic` dependency dropped (Principle 7 already declares reimport the migration story; the scaffold was dead weight); the redundant nested `alldata/` HF clone removed (the root repo's `hf` remote already points at the Space); stale `test.db` removed (`*.db` stays gitignored).
- **Y1, auth IP dams.** `POST /api/auth/login` and `/api/auth/register` are per-IP rate-limited (separate budgets, `settings.auth_rate_limit_login=10` / `auth_rate_limit_register=5` per minute) with 429 + ready-made `Retry-After`, checked before any DB work. Every attempt counts, success or not.
- **Y2, execute per-IP dam.** `POST /api/execute` gains `settings.execution_ip_rate_limit` (30/min) so one host can't rotate throwaway accounts to evade the per-user cap. New `deps.client_ip` reads `X-Forwarded-For` first (the HF proxy masks the socket peer), socket peer as fallback.
- **Y3, global concurrency gate.** All execution dispatch runs under a module-level `asyncio.Semaphore` (`execution_max_concurrent=2`), bounding how many runs the *host* executes at once — the Docker sandbox bounds each run, this bounds the host.
- **Y4, `load(name)` dataset resolution fixed.** The runner passes the absolute curated-dataset dir via `ALLODATA_DATASET_DIR` (local fallback) and mounts `seed/datasets` read-only into the sandbox; the injected `load()` checks that path first. Dataset loading now works in the Docker sandbox, the local fallback, and the HF Space, where it previously depended on a favorable cwd.
- **Y5, tests.** `tests/test_hardening.py`: auth dam 429 + Retry-After flows, limiter reset hygiene (conftest resets all four per test), env-scrub contract (secrets never leak), and end-to-end smoke tests of the hardened Popen path (hello-world + timeout kill), matplotlib-gated so a bare dev install skips cleanly.
- **Y6, durable external DB on the Space.** `hf_start.sh` distinguishes the default SQLite file (seed once per fresh container) from an external `DATABASE_URL` — the durable case runs the idempotent importer on every boot with 3 retries, so a Neon/Supabase Postgres secret makes accounts, progress, and forks survive restarts. `.env.example` documents the `postgresql+asyncpg://…?sslmode=require` shape; `docs/deploy-huggingface.md` gained a Part D walkthrough (the guide's stale "ask and I'll wire it" bullet was corrected).
- **Y7, keep-alive + DB-aware health.** `GET /api/health` now round-trips `SELECT 1` through the engine — 200 `{"status":"ok","database":"ok"}` or 503 with detail — and `.github/workflows/keepalive.yml` pings it every 30 minutes (GitHub cron + manual dispatch). That keeps the free Space from sleeping after 48 idle hours, and an outage (Space down *or* database unreachable) surfaces as a red Actions run instead of a silent failure in Space logs.
- **Y8, asyncpg `sslmode` translation.** First real-world boot with an external `DATABASE_URL` failed with `TypeError: connect() got an unexpected keyword argument 'sslmode'`. Mechanism: SQLAlchemy splits the URL into kwargs and calls `asyncpg.connect(**kw)` directly, bypassing asyncpg's DSN parser (which *does* understand `sslmode` — the earlier offline check had used it); `asyncpg.connect()` only accepts an `ssl` argument. `_engine_setup` in `database.py` now translates `sslmode` → an `ssl` SSLContext with libpq semantics (`require` = encrypt, no verification; `verify-ca`/`verify-full` keep verification; `disable` = off) and strips `channel_binding` with a logged warning; SQLite and sync-psycopg URLs pass through untouched. Also added `asyncpg` to the Space `Dockerfile` (the image installs a hardcoded pip list, not `pyproject.toml`, so the driver the external-DB path needs was missing from the image entirely — the Space's first failure mode). Regression tests in `test_hardening.py` cover the translation, the strip, and the SQLite no-op.
- **Y9, Postgres DDL + pool hygiene.** With the connection live, the *next* boot failed inside `create_tables`: `user_progress` declares `server_default="'{}'"` on its JSON sidecars, and SQLAlchemy renders server-default strings verbatim into DDL — SQLite tolerated the resulting `DEFAULT '''{}'''` but Postgres parses it as the literal text `'{}'` and rejects it as invalid JSON (`Token "'" is invalid`). Defaults corrected to plain `"{}"` (and the same embedded-quote pattern fixed in `fork.py`'s `markdown_source`, which rendered a one-quote-char default); a DDL-compile sweep over all 10 tables × Postgres/SQLite dialects now guards against any recurrence of triple-quoted defaults. Also: Neon drops idle connections and the pool was handing out dead ones (`InterfaceError: connection is closed` on health checks) — `pool_pre_ping=True` on the Postgres engine. Both fixes are SQLite-visible no-ops; the whole class was invisible locally because SQLite is lax about default literals.

**Retrospective.**
- **The trust boundary was the wrong one.** S1's threat model treated the Docker sandbox as the boundary and the fallback as "dev-only"; the HF deployment made the fallback the *production* path. The fix follows the same shape as S1's posture checks: make the unsafe path survivable (allow-list env, rlimits, group kill, concurrency cap) while keeping the honest warning in the startup logs. The real fix remains a per-request sandbox (Pyodide or a provisioned sandbox image); Y shrinks the blast radius in the meantime.
- **Deployment outcome (2026-08-31).** The external-Postgres path is live on the Space: clean boot with the full 47-topic re-import into Neon, all endpoints green, stable health. The three failures it took to get there (missing driver in the image, `sslmode` kwarg, JSON server-default quoting) were each invisible to local SQLite validation — the live Space is now effectively the strictest member of the test matrix, and the Y6 startup retries + Y7 health probe are what made each failure diagnosable from the logs alone.
- **Per-user limits assumed accounts are expensive.** On an open-registration Space they're free, so a per-IP dam is the actual resource bound; the per-user cap still gives professors their higher tier without letting one host mint accounts to buy throughput.
- **Env allow-lists beat env scrub-lists.** Blocking known-dangerous keys leaks on every new variable; allow-listing the ~15 keys the interpreter and matplotlib need is invariant to what the host exports next. The project already pays this style of cost deliberately (`--strict` import validation); this is the runtime version.
- **The nested `alldata/` clone was a deploy path in disguise.** It was an early Space clone with a stub README whose `origin` is the same Space the root repo's `hf` remote points at — safe to delete, but the check (read its remotes before deleting anything) is the rule: a stray `.git` inside a project is a remote, not garbage.

**Deferred (carry-forward).** The external-Postgres *path* is wired (Y6) — what remains is the owner's click: create the provider DB and set the `DATABASE_URL` (+ `SECRET_KEY`) Space secrets per deploy-guide Part D. Redis-backed limiters still gated on multi-process; Pyodide client-side execution (previously declined in U — revisit only if the host-fallback risk profile changes); the long-standing backlog (schema expansion, vitest harness, d3 slimming) unchanged.

---

## A, Platform basics: recovery, full-text search, admin roles

**Intent.** Three "basic platform functionality" gaps from the original review, all sharpened by the external-DB deployment making accounts *durable* (a lost password now means a lost account, not a disposable one): no password recovery, no profile editing, no way to assign the roles that gate `/review`, and search that only saw topic titles. (The letter A was the next unused ledger letter — A–X were exhausted.)

**Shipped.**
- **A2, account settings + recovery codes.** No email infrastructure exists, so recovery is a **single-use code**: `POST /api/users/me/recovery-code` generates `xxxx-xxxx-xxxx-xxxx` (only the bcrypt hash is stored, plaintext shown exactly once, generation rotates), and `POST /api/auth/recover` exchanges email + code for a login token behind its own per-IP dam. Password change (`PATCH /api/users/me/password`) requires the current password and invalidates any outstanding code; profile updates (`PATCH /api/users/me`) cover display name / bio / institution. Frontend: `/settings` page (three sections, copy-once code UI) plus a "Forgot password? Use a recovery code" flow inside the sign-in modal. `users.recovery_code_hash` is an additive column — self-heal handles it on both DBs.
- **A3, admin user management + first-admin bootstrap.** New ADMIN-only router `/api/admin` (`GET /users`, `PATCH /users/{id}/role`, `PATCH /users/{id}/active`) with the **last-admin guard** — the one real invariant, since a platform whose only admin demotes themselves has no way back in (no email, no CLI on HF). Role/active changes are logged with actor + target. First-admin bootstrap: set `ADMIN_EMAIL`; the seed importer promotes that account idempotently on boot. Frontend: `/admin/users` (self-gating table, role dropdown, deactivate/reactivate), "Manage users" popover link, `is_active` surfaced on `UserResponse`.
- **A1, full-text search over topic content.** `GET /api/graph/search` now also matches `content_blocks.content`: title matches rank first (unchanged H6/L4 logic), body matches fill the remaining slots with `matched_in="body"` and a ±60-char `snippet`. Deliberately a portable `LIKE` join rather than FTS5/tsvector — at 47 topics the scan is microseconds, and two dialect-specific index systems would buy nothing user-visible. `SearchDropdown` renders snippets under body matches.
- **Tests.** `tests/test_cycle_a.py` (16): full recovery lifecycle (generate → recover → reuse fails), password change invalidating codes, wrong-code 401, profile update paths, admin role gates, last-admin guard, self-deactivation guard, deactivate→login-fails, body-match snippets, title-over-body ranking, **and the K7 snapshot endpoint contract** (progress aggregation + private-sidecar non-leak + 404). Suite: **83 passing**.
- **A4, search error surfacing.** A failed search request used to render "No topics found" — indistinguishable from a true empty result (exactly what happened during the pg_trgm outage). `SearchDropdown` now tracks fetch failure and shows "Search is temporarily unavailable."
- **A5, Playwright e2e harness.** `frontend/e2e/` + `npm run e2e`: the suite runs against the **production-shaped stack** — built SPA served by uvicorn on :8000 with a throwaway seeded SQLite DB (`e2e.db`), not the dev proxy. 8 specs: home renders + graph canvas boots, home-hero and navbar Ctrl-K search navigate to topics, the body-search API contract (`matched_in="body"` + snippet), and the **full account lifecycle in a real browser** — register → profile edit → password change → generate recovery code → sign out → recover-and-sign-in. Browser install (once): `npx playwright install chromium`.
- **A6, content coverage report.** `backend/services/coverage_service.py` computes, from the live DB: per-topic interactive-block coverage (decision / playground / code / plot), prerequisite-edge connectivity (`required_by_count` — "orphan" = published topic nothing builds on), metadata completeness (recall prompt, summary), and domain/difficulty/dataset distributions. Two consumers: `python -m seed.import_seed --report` (ASCII-safe CLI rendering — redirected Windows consoles are cp1252 and choke on box-drawing characters) and `GET /api/admin/coverage` (ADMIN) feeding `/admin/coverage`, a self-gating page with stat cards, gap lists deep-linking to topics, and a per-topic ✓/✗ table. First real run against the 47-topic catalog: **decision coverage 17/41, playground 24/41, 16 orphan topics, 1 of 4 datasets in use** — exactly the "what to write next" signal the report exists for. (Note: "B3" in the roadmap discussion = this A6; the ledger letter space wrapped to A.)
- **A7, local-fallback memory fix — the hardening met the scientific stack.** Every lesson simulation on the Space died at 141 ms with `OpenBLAS error: Memory allocation still failed after 10 retries`. Cause: Y3's `RLIMIT_AS = 512 MB` caps **virtual** address space, and OpenBLAS (pulled in by numpy on import) reserves large virtual mappings per thread — the cap that guards against memory bombs also starved the scientific stack it was protecting. Two-part fix: `_sandbox_env` pins BLAS/OpenMP to a single thread (`OPENBLAS_NUM_THREADS=1` etc. — kills both the virtual-memory blowup and CPU oversubscription under the A3 concurrency gate; also applied to the Docker sandbox via `-e`), and `sandbox_local_memory_mb` rises 512 → 2048 with a comment explaining the virtual-vs-resident distinction. Windows can't enforce rlimits, so local tests never saw it — the third live-only failure, and the second one caused by the hardening itself rather than missed by it. Lesson recorded: hardening a runtime that hosts *numpy lessons* must be validated by running an actual lesson, not just hello-world.
- **A8, SEO: topic meta injection + crawler surfaces** (roadmap "B4"). The SPA gave every route the same default `<title>`, so 47 pages of real content were invisible to crawlers and link previews — full SSR deliberately out of scope. Instead: the SPA fallback injects a topic's own `<title>`, description, OG/Twitter tags, and a canonical URL into the in-memory `index.html` template for `/topic/{slug}` requests (one published-topic query; unpublished slugs fall back to the default head — no existence leak). New `/sitemap.xml` (all published topics + static routes, `_meta` domains excluded, 5-min cache) and `/robots.txt` (disallow `/api/`, `/admin/`, `/review/`, `/settings/`; Sitemap reference). Canonical origin: `SITE_URL` env overrides forwarded-header derivation. *(roadmap "B4" = this A8.)*
- **A9, e2e in CI** (roadmap "B5"). The Playwright suite joined `.github/workflows/ci.yml` as a third job: install backend + frontend deps, `npx playwright install --with-deps chromium`, `npm run e2e` (build + seed + boot + test), failure traces uploaded as artifacts (7-day retention). The live-only failure class (pg_trgm, the DDL defaults, OpenBLAS under rlimit) is exactly what this closes: the e2e stack is production-shaped, so the next Postgres/runtime surprise fails a PR instead of a deployed Space. Local equivalent unchanged: `npm run e2e`.
- **A10, first-party analytics.** `usage_events` table, one row per (day, kind, slug) with a hit counter, upserted; **no IPs, no user IDs, no sessions** (asserted by test). `POST /api/track` (no auth, anonymous readers count; kinds whitelisted to topic_view/run_click/decision_pick; slug validated against the topics table; per-IP dam at 120/min) and `GET /api/admin/analytics` (per-topic views/runs/picks over a 1 to 90 day window). Frontend: beacons from TopicView (view, post-fetch so 404s don't count), CodeRunner (actual runs, post-auth-gate), DecisionBlock (picks); "Readers (last 30 days)" section on `/admin/coverage`. Beacons are `keepalive` fire-and-forget and can never surface an error to the reader.
- **A11, content batch 1.** Two new decision-driven topics built around the unused datasets: **monty-hall** (monty-hall-runs.csv; the door decision writes `p` and the `binomial_pmf` consequence plot redraws live: stay 1/3, fifty-fifty trap 1/2, switch 2/3) and **base-rates** (medical-test-results.csv; 99%-accurate test on a 1-in-1,000 condition, `population_dot_grid` consequence, PPV ≈ 9% computed from raw patient records). Plus recall prompts for the two topics missing them (bayes-theorem, shape-of-statistics). **Importer fix found by the batch:** the meta-update block sat *after* the skip-if-content-exists `continue`, so recall_prompt/dataset/layer edits to already-shipped topics never propagated. Moved before the skip, per principle 7 (meta.yaml is the source of truth; reimport is the migration story). Post-batch coverage: **recall 43/43, decision 19/43, datasets 3 of 4 in use**.
- **A11 fix, timeout cleanup robustness.** The timeout smoke test flaked under load with `PermissionError` during `TemporaryDirectory` cleanup: the killed child holds handles for a beat after the process-group kill. Real bug, not test noise: the cleanup error fired *after* the result was computed and would have 500'd the execute response. Both local-fallback temp dirs now use `ignore_cleanup_errors=True`; the OS reaps the leftover in %TEMP%.
- **B2, contract fixes + reviewer notes.** (1) The injected `load()` helper returned a pandas DataFrame when pandas was installed and record dicts otherwise, so the monty-hall topic crashed on the live Space (`for g in games` iterated column names) while dev stayed green. One contract now: a list of dicts with real bools/numbers in both branches, regression-tested through the real executor. (2) Domain-filtered graph views anchor edge-less members to their family root with a dashed visual-only link (and keep only that family's root, not all six). (3) The graph search chip falls back to full-text search when no title/slug matches. (4) Reviewer notes work in both directions: accept takes an optional note, and the fork author sees the latest note (accepted or rejected) on ForkView/ForkEditor. (5) Decision retrofits batch 2a under the new anti-template rules, using the "commit, then explore" pairing (decision opens gear 3 and writes the state, the existing playground explores the consequence): expectation (three tickets with EVs 1.0/1.2/1.0, the pick slides the balance point), central-limit-theorem (two researchers' sample means, the pick writes n and the curve tightens to σ/√n), correlation (a y = x² cloud, the pick flattens the fit line to r = 0), p-values (the "no effect" acquittal trap, the pick widens or tightens the null). CLT's top plot now binds n so the consequence is visible. Decision coverage 19/43 → 23/43. Enabler: content edits to shipped topics never reimported (the skip-if-exists guard blocks them, the A11 fix only freed meta fields), so `--refresh-content` now gives the operator the migration story principle 7 promised: boot behavior unchanged, edits land when the flag is run.
- **C1, graph integrity: edges for the missed dependencies, terminals for the deliberate endpoints.** The coverage lens flagged 18 orphans; cross-referencing `schema.yaml` showed two different problems wearing one label. **Six genuine missed edges** landed, each already taught in prose: independence→LLN (the law's "iid" fine print), LLN→CLT (the companion-theorem link), binomial→poisson (the limit the Poisson derivation performs), f-distribution→multiple-regression (the overall F test), statistical-power→ab-testing (designing to a minimum detectable effect), data-wrangling→EDA (clean before you explore). **Eleven topics are intentional chain endpoints** (exponential, t-tests, bayesian-inference, model-diagnostics, logistic-regression, regularization, ab-testing, bias-variance-tradeoff, missing-data, base-rates, monty-hall) and now carry `terminal: true` in the schema; the coverage lens reports them as `terminals`, not orphans, so the orphan list only ever means a real gap. Enabler fix: the additive schema merge only created edges for *new* topics, so new prerequisite declarations on existing topics silently no-op'd forever (same class as the A11 meta-field bug); a C1 sync pass now adds missing edges and syncs the terminal flag on every reimport, idempotently. Fourth dataset came along: `coin-flips-1000` now powers LLN's gear-5 running-fraction demo (`load()` + dataset chip + `dataset:` field), datasets-in-use **4/4**.
- **C2, the learning loop closes.** The three pieces that never touched are now wired. (1) **Decisions feed the SM-2 schedule**: every decision pick now calls `recordReview` on the topic (correct first pick = quality 5, a miss or "show me the answer" = 1, no-`correct` blocks = 3), so the 42 decisions function as assessment and the spaced-repetition prompt reacts to demonstrated recall instead of only self-ratings. Syncs to the server through the existing M1 pipeline. (2) **`/misconceptions`, the consolidated catalog** (the H10 backlog item): new public `GET /api/misconceptions` (one query, misconception × published topic, ordered domain → topic) and a `/misconceptions` page grouping every wrong-belief → correction card by topic, linked from the navbar. (3) **`fill_in`, the last missing interactive gear variant** (the I7 backlog item): a progressive-reveal derivation block for the formal layer, same numbered body as `step_through` but every step hidden behind its own "think it through, then reveal" placeholder until asked for. Full stack: parser (`meta.steps`), `BlockType.FILL_IN`, `FillIn` renderer wired through `BlockRenderer` (so SlideView gets it free), fork-editor toolbar scaffold, confusion-flaggable, authoring-doc section, parser test. First usage: CLT's formal gear (center → scale → shape). **Rider (C5):** the backup runbook landed in `docs/deploy-huggingface.md`: `pg_dump` → nightly artifact workflow → `pg_restore` rehearsal, because the external database is the platform's only durable state and an unrestored backup is a hope.
- **C1 hotfix, live-boot failure on Postgres.** The Space failed to boot three times: the self-heal emitted `ALTER TABLE topics ADD COLUMN terminal BOOLEAN NOT NULL DEFAULT 0`, and Postgres rejects an integer default for a boolean column (SQLite takes it, so every local test stayed green — the standing live-only failure class again). Root cause: `_self_heal_columns` passes the model's raw `server_default="0"` straight into the DDL. `tour` never triggered this because `create_all` made that column; `terminal` was the first self-heal ALTER on the live Postgres. Fix: boolean defaults normalize per dialect in the self-heal (`DEFAULT FALSE` on Postgres, `DEFAULT 0` on SQLite), pinned by `tests/test_self_heal.py`. Any future Boolean column is safe. **Follow-up 500 in the same pass:** the new `/api/misconceptions` endpoint first 500'd on `order_by(Misconception.sort_order)` (a column the model never has), then returned an empty catalog once fixed. Real root cause: the endpoint read the `misconceptions` **table**, which is empty on every real DB because the entire catalog authors misconceptions as `misconception_inline` content blocks (the legacy non-inline form that populates the table is used by zero topics, and `misconception_count` on graph nodes has been silently 0 forever for the same reason). The endpoint now reads the inline blocks — the same rows the topic pages render — and parses the body into title / wrong belief / correction, rendered as markdown on the page. Pinned by `tests/test_misconceptions.py` against real inline blocks.
- **C3, reader-facing growth: the analytics start paying rent.** (1) **Trending on Home**: public `GET /api/graph/trending`, the top-5 topics by view count over the last 7 days rendered as a pill strip under the hero. The section hides itself until beacons exist, so a fresh deploy never shows an empty box, and drafts drop off the strip silently. Aggregate counts only, the no-PII posture holds. (2) **Guided tracks**: three curated reading orders (A/B testing from scratch, Read any regression output, Bayes without tears) defined in `seed/tracks.yaml` and served by `GET /api/tracks`, which resolves every slug against the live DB and drops unknown/unpublished ones, so a track can never dead-end. Rendered on `/path` as "Guided tracks". No new content, the tracks are pure navigation over the 42-topic catalog. **Deferred from this cycle:** PWA/service worker (needs a real SW decision, not a bolt-on manifest), the full WCAG pass.
- **C4, the contribution loop matures.** (1) **Parse warnings gate the Suggest action**: the fork editor's parser warnings (previously a quiet strip on the preview) now render as an amber banner above the source pane where the author types, and "Suggest to master" is disabled with an explanatory tooltip until they're fixed — a contributor can no longer propose content that would fail the strict import. (2) **`/contribute`**, the loop's front door: how the fork → edit → suggest → review pipeline works step by step, the quality bar (parse clean, six-gear shape, decisions that earn their click, the anti-template rules), and where to start (fix a documented misconception). Linked from the account menu. (3) **Review-queue filters**: a pending/resolved/all cut plus a title/author/slug search over the in-memory queue, so a growing queue stays navigable.
- **C5, the nightly backup workflow.** `.github/workflows/backup.yml`: nightly `pg_dump --format=custom` against the external Postgres (repository secret `DATABASE_URL`), dump uploaded as a 30-day artifact, `workflow_dispatch` for on-demand runs, and a clean no-op with a notice when the secret isn't set. The restore rehearsal procedure stays in the deploy doc; now the runbook has its automation.
- **B2b, decision retrofits batch 2: the whole catalog.** All 19 remaining content topics got gear-3 decisions (bernoulli, normal, poisson, t, chi-squared, f, exponential, law-of-large-numbers, random-variables, sampling-distributions, statistical-power, likelihood, MLE, bayesian-inference, simple-linear-regression, logistic-regression, regularization, bias-variance-tradeoff, cross-validation), each writing state keys an existing plot binds, so every pick moves the picture: the sensor-vs-coin bars, the σ-squeeze, the equidispersion dial, the t's fat tails at df 4, the memoryless wait, the variance-test df (n−1), the ANOVA df puzzle F(3, 20), the gambler's-fallacy spike, the zero-width point probability, what shrinks (SE, not σ), the power lever priced in √n, the likelihood peak at k/n, the MLE at the all-tails boundary, the prior's pseudo-count tug, the OLS mean-point anchor, the −β₀/β₁ boundary, lasso-vs-ridge paths, the overfit diagnosis on the U, and the training-vs-validation gap. Every verdict names what the reader is now looking at; no two verdicts in the same family share an opening move. The one topic still without a decision is `shape-of-statistics`, the `_meta` graph-tour intro that intentionally isn't a six-gear lesson. **Decision coverage 42/43; every standard content topic now commits its reader before it explains.**

**Retrospective.**
- **pg_trgm was the same class of failure as Y9, hiding in a different layer.** Y9 caught Postgres-strict *DDL*; A's first live boot caught a Postgres-only *runtime dependency* — the H6-era `similarity()` call needs the `pg_trgm` extension, which fresh Neon databases don't have, so every search 500'd in production while 83 tests stayed green on SQLite. The fix removed the dependency rather than provisioned around it (a ranked-ILIKE that's dialect-portable and, at 47 topics, just as good). Standing rule going forward: any Postgres-specific SQL in this codebase must either be feature-detected with a fallback or justified in writing; the live Space is part of the test matrix now.
- **The e2e harness paid for itself on its first run.** Three of eight specs failed initially — a strict-mode collision (navbar "Sign in" chip vs. modal submit share a name), a wrong assumption about domain-card labels (`DOMAIN_LABEL` "Probability", not the schema title), and a fragile text-based body-match locator. Every one of those was a selector-robustness lesson absorbed into the specs; the suite that ships is the one that survived contact.
- **Ruff's fix pass caught a bug the test suite didn't.** Rewriting `users.py`'s header dropped the imports the K7 snapshot endpoint still used — no test covers the snapshot endpoint, so `pytest` was green while the module was broken at runtime; only `ruff`'s F821 flags surfaced it. Lesson: a module-level `--fix` sweep is a dependency audit in disguise. The snapshot contract test now exists.
- **The three Y-cycle failures primed T.** Every T design decision leaned on Y's machinery: the recovery endpoint reuses the Y1 limiter pattern, the last-admin guard exists *because* there's no email/CLI recovery on HF, and the `ADMIN_EMAIL` bootstrap closes the "first admin is magic" gap flagged in the original review.
- **Search stayed boring on purpose.** The tempting version of T1 was FTS5 + tsvector with weight curves; the honest version is a LIKE join over a few hundred rows plus title-first ordering. Same principle that kept the authoring surface markdown: match the tool to the scale, not to the imagined scale.

**Deferred (carry-forward).** Account deletion (fork ownership FKs make it a real design question — needs an ownership-transfer or orphaning story, not a bare DELETE); a snapshot-endpoint regression test; pagination on the admin user list past ~500 users; email-based recovery if an email provider ever lands; the standing backlog (schema expansion, vitest harness, d3 slimming) unchanged.
