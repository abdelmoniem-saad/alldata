...
### Quiz block (D3)
Two to four check-yourself questions, asked one at a time with immediate verdicts and teaching responses. YAML body shaped like a decision (`questions:` with `prompt`/`options`/`correct` 0-based index/`response`), validated at import (correct index in range, title required, 2-5 questions). Every answered question fires a review event (quality 5 first-pick-correct, 1 on a miss), making quizzes the density engine of the SM-2 scheduler. Picks live in local state deliberately: re-answering on a revisit is practice. Shipped with quizzes on p-values, hypothesis-testing, and CLT. *(cycle: D3)* `code: seed/import_seed.py` (parser + validation), `frontend/src/components/topic/blocks/QuizBlock.tsx`.

### Exercise block (D3)
Numeric-entry practice with tolerance checking. YAML body: `prompt`, `answer` (number, validated at import), `tolerance`, `unit`, `hint`, `solution`. The check compares numerically (|value - answer| <= tolerance), so 6.09 passes 6.0 ± 0.11. One review event per block (5 on a correct check, 1 on reveal). Hint releases progressively; the solution panel only appears after a commit or reveal. Shipped with exercises on variance (sample SD) and confidence-intervals (interval width). *(cycle: D3)* `code: frontend/src/components/topic/blocks/ExerciseBlock.tsx`.

### Second capstone: the intervention analysis (D4)
`capstone-intervention` on the `daily-metrics.csv` dataset (120 days × 2 regions, committed): a feature launches to one region on day 70, a promo runs in both regions after day 70, plus a viral spike, a control outage, and missing rows. The topic walks the honest arc from naive before/after (+9.6) through raw DiD (+9.2, the control's outage still inside) to the cleaned DiD (+8.3, recovering the 8/day truth), with a decision binding the three estimates onto the empirical histogram, a gated quiz, and a DiD exercise computed from the real cleaned means. Every number quoted in the prose was verified against the shipped CSV. *(cycle: D4)* `code: seed/datasets/daily-metrics.csv`, `seed/topics/data-science-practice/capstone-intervention/`.

### Methodology page (D5)
`/methodology`: how the content is written (six gears, anti-template rules), checked (strict import, per-layer tests, e2e against the built app, figures verified against shipped data), corrected (flag-a-problem and forks), and what the platform does not claim (checked, not certified; association, not causation; the syllabus names the edges). Static page, linked from About and the flag modal. *(cycle: D5)* `code: frontend/src/pages/Methodology.tsx`.

### Flag a problem (D5)
The lightweight correction door: a quiet chip on every topic's bottom bar opens a modal that files a `content_reports` row via the public `POST /api/reports` (anonymous allowed, per-IP dam at 5/min, note length enforced, topic slug validated, signed-in reporters attached via the new `OptionalUser` dependency). Admin triage on `/admin/reports`: open/resolved/dismissed filters, deep-linked topics, and Resolve/Dismiss with an optional closure note (the reviewer-notes pattern). New `content_reports` table, created by `create_all` on boot. *(cycle: D5)* `code: backend/models/report.py`, `backend/api/reports.py`, `backend/api/admin.py` (reports routes), `frontend/src/components/FlagModal.tsx`, `frontend/src/pages/AdminReports.tsx`.

### Public syllabus (D1)
The public curriculum map at `/syllabus`: the whole statistics & data-science course in a learner's order, grouped into 8 areas (Foundations, Distributions, Estimation, Testing, Modeling, Evaluation, Practice). Written units (green ✓, difficulty tag) link into their lessons; planned units (open ○, "planned" chip + a note on what the unit will cover) are declared on purpose, never hidden. The header shows the honest count ("44 of 60 units written · 73% of the map"). Source of truth `seed/syllabus.yaml`; `GET /api/syllabus` resolves topic slugs against the live DB at read time, so a planned unit flips to written the moment its lesson ships, and an unpublished topic drops its unit to planned rather than serving a dead link. In the navbar, in the sitemap. *(cycle: D1)* `code: seed/syllabus.yaml`, `backend/api/syllabus.py`, `frontend/src/pages/Syllabus.tsx`. **D2: all 19 planned units were written and linked** (bootstrap, effect sizes, multiple comparisons, ANOVA, chi-squared test, nonparametric tests, permutation tests, joint distributions, beta/gamma, time series, classification metrics, decision trees, clustering, dimensionality reduction, imbalanced data, feature leakage, causal inference, experimental design, capstone), taking the map to 62/62 written.
...
A **Visual ⇄ Source** toggle in the editor bar (Visual default). Visual mode hides the raw directive syntax entirely: the fork renders as an ordered list of editable **prose** markdown fields and friendly directive **cards** (`SECTION`, `PLOT`, `CALLOUT`, …) with reorder / delete / edit. Edit opens a per-directive **form** (gear label + stage, callout kind + text, state key/value rows, plot parameters, code language + flags, layer select, …), no YAML, no `<!-- block -->`; `decision`/`playground` and any unmapped type fall back to a per-block raw "edit source" hatch. The W toolbar + plot picker become the insert bar; `<!-- layer: … -->` markers show as a compact `LAYER` chip. It's a **pure frontend editing layer over the same `markdown_source` string**: a tokenizer (`contentDoc.parseDoc`) partitions the source into segments whose `raw` concatenates back byte-for-byte (`serializeDoc(parseDoc(x)) === x`), so editing one block rewrites only that segment, the backend parser, the live preview, and the merge-back diff are all unchanged. Source mode is today's raw `content.md` textarea, preserved as the escape hatch. **C8, calm-editor pass:** the same engine with progressive disclosure — row controls (reorder/delete/drag) reveal on hover or keyboard focus and stay faint on touch; prose fields read as document text at rest; the layer selector lives inside the block editor instead of the row head; the flat toolbar is a single "Insert +" menu with described entries grouped Text / Teaching / Interactive / Data; Visual mode's preview pane collapses behind a Preview toggle (Source mode always previews); a one-time dismissible hint states the interaction model. The round-trip guarantee and merge-back diff are unchanged. *(cycle: X, C8; C9 owner revision: the preview pane is open by default, the toggle collapses it)* `code: frontend/src/lib/contentDoc.ts, frontend/src/lib/directiveMeta.ts, frontend/src/components/topic/BlockListEditor.tsx, BlockCard.tsx, blockForms/*`.
...
Bottom-chrome chip on `/topic/:slug`, left of LEARNED. Visible to signed-in and anonymous readers alike (tour topics excepted). Signed in: reads "Fork this topic" (creates a fork, opens the editor) or "Open my fork" when one already exists. Anonymous: clicking opens the global sign-in modal, and the fork is created and opened automatically once the reader signs in (the run-gate pattern). *(cycle: N, C8)* `code: frontend/src/components/topic/ZenChrome.tsx` (search `canFork`), `frontend/src/pages/TopicView.tsx` (`handleForkClick`, `pendingForkRef`), `frontend/src/pages/ForkEditor.tsx` (signed-out gate card).
...
| `POST /api/auth/login` / `POST /api/auth/register` / `GET /api/auth/me` | Account flows. Login/register wired into the navbar's `AuthMenu` in M1; `/me` returns the JWT-validated current user; login/register are per-IP rate-limited. Register accepts an optional self-reported `audience` (student / professor / professional / curious), descriptive only, editable via `PATCH /users/me`; unknown values degrade to None. | seed-era; M1, UI wired; Y1; C9, audience |
...
`hf_start.sh` distinguishes the default SQLite file (seed once per fresh container) from an external `DATABASE_URL` (idempotent re-import on every boot, 3 retries with backoff), so a Neon/Supabase Postgres secret makes accounts, progress, and forks survive Space restarts. The engine layer translates libpq-style URL params for asyncpg: `sslmode` becomes a native `ssl` SSLContext (libpq semantics) and `channel_binding` is dropped with a warning, since SQLAlchemy forwards query params to `asyncpg.connect()` as kwargs and would otherwise raise `TypeError`. JSON/Text `server_default`s are plain literals (no embedded quotes — Postgres parses them strictly where SQLite didn't) and the Postgres engine runs `pool_pre_ping` so Neon's dropped idle connections never surface as `connection is closed`. *(cycle: Y6; Y8, sslmode translation + driver in image; Y9, DDL defaults + pool_pre_ping)* `code: infra/hf_start.sh`, `backend/database.py` (`_engine_setup`, `_asyncpg_ssl`), `backend/models/progress.py`, `backend/models/fork.py`, `.env.example`, `Dockerfile`.
...
`frontend/e2e/` + `npm run e2e`: production-shaped stack (built SPA served by uvicorn with a seeded throwaway SQLite DB), 8 specs covering home/graph rendering, both search surfaces navigating to topics, the body-search API contract, and the full account lifecycle (register → settings → recovery code → sign out → recover-and-sign-in). One-time browser install: `npx playwright install chromium`. Runs in CI as a third job (failure traces uploaded as artifacts). *(cycles: A5 harness, A9 CI)* `code: frontend/playwright.config.ts`, `frontend/e2e/`, `.github/workflows/ci.yml`.

### Content coverage lens
`coverage_service.build_coverage_report` computes per-topic interactive-block coverage (decision / playground / code / plot), prerequisite-edge connectivity (`required_by_count`; "orphan" = published topic nothing builds on), metadata completeness, and domain/difficulty/dataset distributions. Two consumers: `python -m seed.import_seed --report` (ASCII-safe CLI output) and `GET /api/admin/coverage` → the ADMIN-only `/admin/coverage` page (stat cards, gap lists deep-linking to topics, per-topic ✓/✗ table). Topics declared `terminal: true` in `schema.yaml` (intentional chain endpoints) report under `terminals`, not `orphans`, so the orphan list only ever means a real gap. *(cycle: A6; C1, terminals)* `code: backend/services/coverage_service.py`, `backend/api/admin.py`, `seed/import_seed.py` (`--report`), `frontend/src/pages/AdminCoverage.tsx`.
...
The seed importer updated meta fields (recall_prompt, dataset, layers, tour) only for topics *receiving new content*: the block sat after the skip-if-content-exists `continue`, so edits to shipped topics' `meta.yaml` never reached the DB. The update now runs before the skip, honoring principle 7 (meta.yaml is the source of truth; reimport is the migration story). For *content* edits the migration story is explicit: `python -m seed.import_seed --refresh-content` replaces each seeded topic's DB blocks with a fresh parse of its content.md (boot keeps skip-if-exists so API-authored blocks survive restarts). *(cycle: A11; B2a, --refresh-content)* `code: seed/import_seed.py` (content-import loop, `_refresh_content`).
