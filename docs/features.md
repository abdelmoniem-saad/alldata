...
Server-executed code blocks. `simulation` flags the runner with a teal indicator. Optional `editable: true` makes the textarea writable. `auto_run: true` runs once on first scroll-into-view (cached afterward). The `load(name)` helper is injected into the Python preamble so code can pull curated datasets. **S1: execution requires sign-in** — anonymous readers get a "sign in to run code" hint instead of a bare 401, and auto-run is skipped without a token so lessons don't fire doomed requests. *(cycles: earlier — code; I4 — auto_run; K5 — load() helper; S1 — auth)* `code: frontend/src/components/topic/CodeRunner.tsx`, `backend/services/execution_service.py` (search `load`).
