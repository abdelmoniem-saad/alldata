...
**Deferred (carry-forward).** External Postgres via a Space secret (makes accounts/progress/forks durable on the free tier — the single next infrastructure decision); Redis-backed limiters still gated on multi-process; a keep-alive cron against `/api/health`; Pyodide client-side execution (previously declined in U — revisit only if the host-fallback risk profile changes); the long-standing backlog (schema expansion, vitest harness, d3 slimming) unchanged.
