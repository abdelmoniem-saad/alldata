...
<!-- block: gear, n: 3, label: "The cloud r can't see" -->

<!-- block: decision, anchor: corr-quadratic -->
question: |
  The fit line above sits on a cloud generated by $y = x^2$, sampled on
  [-2, 2]. What value does Pearson's r take for this cloud?
options:
  - id: a
    label: "r = +1: the relationship is deterministic"
    writes: { slope: 1.0 }
    response: |
      Deterministic, yes: every point lies exactly on the curve. But watch
      the fit line tilt while the cloud refuses to straighten; r measures
      how well a *straight line* explains the cloud, and the true
      relationship is a curve. The determinism is real; the +1 is not.
  - id: b
    label: "r = 0: the line goes flat"
    writes: { slope: 0.0 }
    response: |
      Exactly. Flatten the line and it plainly misses everything, yet the
      points are glued to a perfect curve. For a symmetric parabola, the
      rises on the right of the mean cancel the falls on the left, the
      covariance lands on zero, and r with it. Perfect relationship, r = 0.
      Plot before you trust r.
  - id: c
    label: "r = -1: strong but negative"
    writes: { slope: -1.0 }
    response: |
      Now the fit line flips downward while the cloud stays a valley
      opening upward: the sign can't even agree with the picture. A
      symmetric U gives the covariance nothing to sum, there's no direction
      to be negative about. r is 0, not -1.
correct: b
<!-- /block -->
